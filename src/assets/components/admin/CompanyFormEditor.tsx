...
import { parseHoursText, validateSchedule } from "../../shared/publicBusinessHours";
...
const PLAN_CONFIG = {
  basic: {
    ...
    perks: ["Nome e categoria", "Localização", "Descrição", "WhatsApp"],
...
// Um item de galeria pode ser uma imagem já salva no Supabase (existing)
// ou um arquivo novo ainda não enviado (new). A ordem do array define a
// ordem de exibição — o índice 0 é sempre a imagem principal/capa.
type GalleryItem =
...
/**
 * Converte o valor salvo em `image_url` (URL simples legada ou JSON-array
 * de múltiplas imagens) numa lista ordenada de URLs para edição.
 */
function parseExistingImageUrls(raw: string | null | undefined): string[] {
  if (!raw?.trim()) return [];
  try {
    const parsed = JSON.parse(raw);
    if (Array.isArray(parsed) && parsed.length > 0) return parsed;
  } catch {
    // não é JSON, trata como URL única
  }
  return [raw.trim()];
}
...
export default function CompanyFormEditor({ mode, businessId }: Props) {
  const navigate = useNavigate();
  const fileInputRef = useRef<HTMLInputElement>(null);

  // No modo criação começamos pela escolha do plano; no modo edição vamos
  // direto para o formulário, já com o plano atual carregado.
  const [step, setStep] = useState<"choose-plan" | "form">(
    mode === "create" ? "choose-plan" : "form"
  );
  ...
  // ── Carrega os dados da empresa no modo edição ─────────────────────
  useEffect(() => {
    ...
    async function loadCompany() {
      ...
      setGallery(
        parseExistingImageUrls(data.image_url).map((url) => ({
  ...
  function handleFileSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const remaining = config.maxImages - gallery.length;
    const toAdd: GalleryItem[] = files.slice(0, remaining).map((file) => ({
      kind: "new",
      file,
  ...
  async function uploadNewImages(items: GalleryItem[]): Promise<string[]> {
    const urls: string[] = [];
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (plan !== "basic" && formData.hours_text) {
      const schedule = parseHoursText(formData.hours_text);
      if (schedule) {
    ...
    setSaving(true);
    setUploading(gallery.some((g) => g.kind === "new"));

    try {
      // O plano pode ter sido rebaixado (ex: Premium → Pro/Basic) desde que as
      // imagens foram carregadas/adicionadas; a galeria salva nunca pode
      // ultrapassar o limite do plano atual, e planos sem imagem (Basic) não
      // devem enviar nem persistir nenhuma.
      const cappedGallery = plan === "basic" ? [] : gallery.slice(0, config.maxImages);
      const imageUrls = await uploadNewImages(cappedGallery);
      setUploading(false);

      // Destaque não é um controle manual: só existe (e é automático) no
      // plano Premium, conforme os próprios planos declaram.
  ...
  // ── Passo 1 (apenas criação): escolha de plano ─────────────────────
  if (step === "choose-plan") {
    return (
  ...
  // ── Passo 2: formulário (criação e edição) ─────────────────────────
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        ...
        {mode === "edit" && (
          <select
            ...
            className="rounded-full border border-slate-200 px-3 py-1 text-xs font-medium text-slate-600 outline-none"
          >
            {(["basic", "pro", "premium"] as Plan[]).map((p) => (
              <option key={p} value={p}>
                Mudar para {PLAN_CONFIG[p].label}
              </option>
            ))}
          </select>
        )}
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          {plan !== "basic" && (
            <div className="md:col-span-2">
                    ...
                    <div key={i} className="relative h-24 w-24 overflow-hidden rounded-xl border border-slate-200">
                      ...
                      {i === 0 ? (
                        ...
                        <button
                          ...
                          className="absolute bottom-1 left-1 flex items-center gap-1 rounded-md bg-black/60 px-1.5 py-0.5 text-[10px] font-medium text-white transition hover:bg-brand-500"
              ...
              {/* Upload area */}
              {gallery.length < config.maxImages && (
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="text-sm font-medium">
                    {gallery.length === 0
                      ...
                      : `Adicionar mais (${gallery.length}/${config.maxImages})`}
                  </span>
              ...
              <p className="mt-2 text-xs text-slate-400">
                ...
                {config.maxImages > 1 && " A primeira imagem da lista é usada como capa — use \"Tornar capa\" para trocar."}
              </p>
            </div>
          )}

          {/* Destaque — automático e exclusivo do Premium, conforme os planos */}
          {plan === "premium" && (
            <div className="md:col-span-2">
